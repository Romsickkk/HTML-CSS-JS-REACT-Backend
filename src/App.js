...
function Counter() {
  ...
  return (
    <div>
      <span style={{ ontSize: "40px" }}>{count}</span>
      <button
...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && isValidHttpUrl(source) && category && textLength <= 200) {
      ...
      // };
      //3. Uppload fact to Supabase and resive
      setIsUploading(true);
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      ...
      <input
        type="text"
        placeholder="Trustwotrhy spurce..."
        name=""
        id=""
        value={source}
...
function FactList({ facts, setFacts }) {
  // TEMPORARY
  if (facts.length === 0) return <p className="message">No facts yet</p>;
...
function Fact({ fact, setFacts }) {
  ...
  const isDisputed =
    fact.votesIntersting + fact.votesMingblowing < fact.votesFalse;
  ...
  return (
    <li className="fact">
      ...
      <div className="vote-buttons">
        ...
        <button
          onClick={() => handleVote("votesMingblowing")}
          disabled={isUpdating}
        >
          🤯 {fact.votesMingblowing}
        </button>
